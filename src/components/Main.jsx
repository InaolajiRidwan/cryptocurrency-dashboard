...
export default function Main({ filteredCrypto, cryptoData, error }) {
  ...
  return (
    ...
    //           <p className="font-bold text-sm">${crypto.high_24h.toLocaleString()}</p>
    //           <div
    ...
    //   </div>
    // </div>
    <div className="custom-theme flame p-6 md:p-10 grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
  ...
  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
    {filteredCrypto.map((crypto) => (
      <div
        key={crypto.id}
        className="flex justify-between p-4 shadow-md rounded-lg bg-white w-full max-w-sm mx-auto border border-gray-200"
      >
        {/* Left */}
        <div className="flex flex-col justify-between">
        ...
        <div className="flex flex-col justify-between items-end">
          ...
          <button
            className="bg-blue-500 text-white rounded px-3 py-1 text-xs mt-2 hover:bg-blue-600 transition"
            onClick={() => handleWatchList(crypto)}
          >
            Watchlist
          </button>
  ...
  <div className="w-full space-y-6">
    ...
    <div className="hidden sm:block">
      <h1 className="text-center font-extrabold text-2xl p-4 underline text-gray-800">
        Watchlist
      </h1>
    ...
    <div className="sm:hidden space-y-4">
      <h1 className="text-lg font-bold text-center text-gray-800">Watchlist</h1>
      {watchList.map((crypto, index) => (
        <div
          key={crypto.id}
